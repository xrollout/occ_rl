"""
Evaluate 3M step model success rate over 100 episodes.
"""
import os
import sys
import torch
...
def main():
    checkpoint_path = './output/ppo_training_output_3m/checkpoint_3002368.pt'
    device = 'cpu'
    n_episodes = 100

    print('=' * 80)
    print('EVALUATING 3M STEP MODEL')
    print('=' * 80)
    ...
    print()
    print('=' * 80)
    print('FINAL RESULTS (3M Step Model)')
    print('=' * 80)
    print(f'Total timesteps trained: 3,002,368')
    ...
    if successes > 0:
        ...
        print(f'Medium reward (successful): {np.median(success_rewards):.2f}')
    print()

    # Save results
    os.makedirs('./output/evaluation_results', exist_ok=True)
    with open('./output/evaluation_results/3m_model_results.txt', 'w') as f:
        f.write(f'Total timesteps: 3,002,368\n')
